...
LOGO_IMAGEM = "logo2.png"      # se não existir, procuramos outra imagem na pasta
...
def anim_worker():
    ...
    def encontrar_logo_na_pasta():
        try:
            ...
            for f in arquivos:
                lf = f.lower()
                if lf.startswith("logo2.") and lf.endswith(exts):
                    return f
            for f in arquivos:
    ...
    if logo_path:
        try:
            ...
            pad = 8
            max_w = LOGO_BOX_W - 3 * pad
            max_h = LOGO_BOX_H - 3 * pad
...
def _audio_callback(indata, frames, time_info, status):
    ...
    try:
        if status:
            # status pode carregar underrun/overrun; mantemos simples
            pass
        if rec is not None:
