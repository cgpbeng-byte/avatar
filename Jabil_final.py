...
# ---- LOGO (box + imagem) ----  << padrão >>
LOGO_IMAGEM = "logo.jpeg"      # se não existir, procuramos outra imagem na pasta
...
# === CONTEXTO (persona) — atualizado conforme pedido ===
historico = [
    {"role": "system",
     "content": "Você é Maives, assistente de engenharia da Jabil. Responda em português do Brasil, em uma única frase, com tom profissional, direto e objetivo."}
]
ocupado = False
...
CHANNELS = 1
DTYPE = 'int16'
# Pasta sugerida do modelo (renomeie conforme o que você baixou)
VOSK_MODEL_DIR = "vosk-model-small-pt-0.3"

# === NOVO: estado da gravação ===
vosk_model = None
rec = None
gravando = False
sd_stream = None
...
def anim_worker():
    ...
    pygame.display.set_caption("MAIVES-Jabil")
...
# === NOVO: gravação e transcrição (toggle) ===
def toggle_gravacao():
    global gravando
    if gravando:
        parar_gravacao()
...
def iniciar_gravacao():
    ...
    if not SD_OK or not VOSK_LIB_OK:
        rotulo_resposta.config(text="STT indisponível: instale 'sounddevice' e 'vosk'.")
        return
    if vosk_model is None:
        rotulo_resposta.config(text="Modelo Vosk pt-BR não encontrado. Coloque a pasta do modelo ao lado do script.")
        return
    if gravando:
        return
    try:
        ...
        sd_stream.start()
        gravando = True
        btn_falar.config(text="⏹ Parar")
        rotulo_resposta.config(text="Gravando... fale agora.")
    except Exception as e:
        rotulo_resposta.config(text=f"Erro ao iniciar gravação: {e}")
...
def parar_gravacao():
    ...
    try:
        ...
        if not texto:
            rotulo_resposta.config(text="Não entendi sua fala. Tente novamente.")
            return
...
def _audio_callback(indata, frames, time_info, status):
    # Alimenta o reconhecedor com chunks de áudio
    try:
        if status:
            pass
        if rec is not None:
...
# Botão de fala (push-to-talk)
btn_falar = tk.Button(frame_botoes, text="🎤 Falar", command=toggle_gravacao)
...
# Carrega o modelo Vosk (se disponível)
